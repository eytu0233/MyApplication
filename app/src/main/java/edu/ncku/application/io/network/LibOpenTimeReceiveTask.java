...
public class LibOpenTimeReceiveTask extends JsonReceiveTask {
    ...
    private static final String JSON_LIB_INFO_URL = "http://140.116.207.24/libweb/libInfoJson.php";
    ...
    private void parsingAllJson() throws Exception {
        ...
        for(int titleIndex = 0; titleIndex < titles_cht.length(); titleIndex++){
            ...
            JSONArray services = content_cht.getJSONArray(title);
        ...
        for(int titleIndex = 0; titleIndex < titles_eng.length(); titleIndex++){
            ...
            JSONArray services = content_eng.getJSONArray(title);
